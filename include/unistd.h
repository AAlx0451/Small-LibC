...
/* VARIABLES */
# if defined(_DARWIN_C_SOURCE) || defined(_GNU_SOURCE) ||                                          \
     (defined(_POSIX_C_SOURCE) && _POSIX_C_SOURCE >= 2) || defined(_XOPEN_SOURCE)
extern char *optarg;
extern int optind, opterr, optopt;
# endif /* _DARWIN_C_SOURCE || _GNU_SOURCE || (_POSIX_C_SOURCE && _POSIX_C_SOURCE >= 2) ||         \
           _XOPEN_SOURCE */
...
char *ttyname(int fd);
# endif /* _DARWIN_C_SOURCE || _GNU_SOURCE || (_POSIX_C_SOURCE && _POSIX_C_SOURCE >= 1) ||         \
           _XOPEN_SOURCE */
...
# if defined(_DARWIN_C_SOURCE) || defined(_GNU_SOURCE) ||                                          \
     (defined(_POSIX_C_SOURCE) && _POSIX_C_SOURCE >= 199309L) || defined(_XOPEN_SOURCE)
int fsync(int fd);
# endif /* _DARWIN_C_SOURCE || _GNU_SOURCE || (_POSIX_C_SOURCE && _POSIX_C_SOURCE >= 199309L) ||   \
           _XOPEN_SOURCE */
...
void sync(void);
int fchdir(int fd);
# endif /* _DARWIN_C_SOURCE || _GNU_SOURCE || (_POSIX_C_SOURCE && _POSIX_C_SOURCE >= 200112L) ||   \
           _XOPEN_SOURCE */
...
#  define _SYSCALL_COUNT_ARGS(...)                                         _SYSCALL_GET_NTH_ARG(__VA_ARGS__, 8, 7, 6, 5, 4, 3, 2, 1, 0, 0)
#  define _SYSCALL_CONCAT_IMPL(name, count)                                name##count
#  define _SYSCALL_CONCAT(name, count)                                     _SYSCALL_CONCAT_IMPL(name, count)

#  define syscall(...) _SYSCALL_CONCAT(syscall, _SYSCALL_COUNT_ARGS(__VA_ARGS__))(__VA_ARGS__)
