...
int main(int argc, char *argv[]) {
    ...
    if(fwrite(&frl, sizeof(frl), 1, fp) != 1)
        goto write_error;
    if(is_wchar) {
        if(rt_count > 0 && fwrite(rt_ranges, sizeof(FileRuneEntry), rt_count, fp) != (size_t)rt_count)
            goto write_error;
        if(ml_count > 0 && fwrite(ml_ranges, sizeof(FileRuneEntry), ml_count, fp) != (size_t)ml_count)
            goto write_error;
        if(mu_count > 0 && fwrite(mu_ranges, sizeof(FileRuneEntry), mu_count, fp) != (size_t)mu_count)
            goto write_error;
    }
