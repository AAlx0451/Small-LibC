...
int rt_count = 0, rt_cap = 0;
FileRuneEntry *ml_ranges = NULL;
int ml_count = 0, ml_cap = 0;
FileRuneEntry *mu_ranges = NULL;
int mu_count = 0, mu_cap = 0;
...
static void add_range(FileRuneEntry **arr,
                      int *count,
                      int *cap,
                      uint32_t min,
                      uint32_t max,
...
static int parse_unicodedata(const char *filename, int auto_ascii)
{
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (fields[13][0])
            lower_map[cp] = strtol(fields[13], NULL, 16);
        if (fields[12][0])
            upper_map[cp] = strtol(fields[12], NULL, 16);
    }
    fclose(f);
    ...
    if (auto_ascii) {
        for (int i = 0; i < 128; i++) {
            ...
            if (i >= 0x09 && i <= 0x0D)
                m |= RL_CTYPE_S;
            if (i == ' ')
                m |= RL_CTYPE_S | RL_CTYPE_B | RL_CTYPE_R;
            if (i == '\t')
                m |= RL_CTYPE_B;
            if (i >= 0x21 && i <= 0x7E)
                m |= RL_CTYPE_R | RL_CTYPE_G;
            ...
            if ((m & RL_CTYPE_G) && !(m & (RL_CTYPE_A | RL_CTYPE_D)))
                m |= RL_CTYPE_P;
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
            ...
            if (strcmp(val, "wchar") == 0) {
                ...
                fprintf(stderr,
                        "Error: Invalid value for --size. Expected 'char' or 'wchar', got '%s'.\n",
                        val);
                return EXIT_FAILURE;
            }
            ...
            if (strcmp(val, "yes") == 0) {
                ...
                fprintf(
                    stderr,
                    "Error: Invalid value for --auto-ascii. Expected 'yes' or 'no', got '%s'.\n",
                    val);
                return EXIT_FAILURE;
            }
    ...
    if (!is_wchar) {
        ...
        while (start < MAX_UNICODE) {
            ...
            uint32_t end = start;
            while (end + 1 < MAX_UNICODE && type_map[end + 1] == mask)
                end++;
        ...
        start = 256;
        while (start < MAX_UNICODE) {
            if (lower_map[start] == start) {
                start++;
                continue;
            }
            int32_t delta = (int32_t)lower_map[start] - (int32_t)start;
            uint32_t end = start;
            while (end + 1 < MAX_UNICODE && lower_map[end + 1] != (end + 1) &&
        ...
        start = 256;
        while (start < MAX_UNICODE) {
            if (upper_map[start] == start) {
                start++;
                continue;
            }
            int32_t delta = (int32_t)upper_map[start] - (int32_t)start;
            uint32_t end = start;
            while (end + 1 < MAX_UNICODE && upper_map[end + 1] != (end + 1) &&
    ...
    if (!fp) {
        fprintf(
            stderr, "Error: Cannot create output file '%s': %s\n", output_file, strerror(errno));
        goto cleanup;
    }

    if (fwrite(&frl, sizeof(frl), 1, fp) != 1)
        goto write_error;
    if (is_wchar) {
        if (rt_count > 0 &&
            fwrite(rt_ranges, sizeof(FileRuneEntry), rt_count, fp) != (size_t)rt_count)
            goto write_error;
        if (ml_count > 0 &&
            fwrite(ml_ranges, sizeof(FileRuneEntry), ml_count, fp) != (size_t)ml_count)
            goto write_error;
        if (mu_count > 0 &&
            fwrite(mu_ranges, sizeof(FileRuneEntry), mu_count, fp) != (size_t)mu_count)
            goto write_error;
    }
