...
static int __tz_parse_body(int fd, tz_db_t* db, int is_v2)
{
    ...
    if (is_v2)
    {
        for (i = 0; i < db->header.tzh_timecnt; i++)
        {
            ...
            if (read(fd, &db->transitions[i], 8) != 8) return -1;
    ...
    for (i = 0; i < db->header.tzh_typecnt; i++)
    {
        ...
        if (read(fd, &raw, sizeof(raw)) != sizeof(raw)) return -1;
...
tz_db_t* tz_load(const char* filepath)
{
    ...
    if (version == 2)
    {
        if (__tz_skip_data(fd, &db->header) != 0) goto error;
        
        if (read(fd, &db->header, sizeof(struct __tz_header)) != sizeof(struct __tz_header)) goto error;
        if (memcmp(db->header.magic, "TZif", 4) != 0) goto error;
...
int tz_lookup(const tz_db_t* db, int64_t timestamp, tz_result_t* result)
{
    ...
    if ((uint32_t)type_idx >= db->header.tzh_typecnt) return 0;
