...
double pow(double x, double y) {
    ...
    if (y == 0x0.0p+0) return 0x1.0p+0;
    if (x == 0x1.0p+0) return 0x1.0p+0;

    /* NaN propagation */
    if (isnan(x) || isnan(y)) return x + y;

    if (isinf(y)) {
        if (x <= -0x1.0p+0 && x >= -0x1.0p+0) return 0x1.0p+0; /* x == -1.0 */
        if (fabs(x) < 0x1.0p+0) return (y < 0x0.0p+0) ? HUGE_VAL : 0x0.0p+0;
        if (fabs(x) > 0x1.0p+0) return (y < 0x0.0p+0) ? 0x0.0p+0 : HUGE_VAL;
    }
    ...
    if (isinf(x)) {
        if (x > 0x0.0p+0) {
            ...
            if (y > 0x0.0p+0) return (y_i == 1) ? -HUGE_VAL : HUGE_VAL;
            if (y < 0x0.0p+0) return (y_i == 1) ? -0x0.0p+0 : 0x0.0p+0;
        }
    }
