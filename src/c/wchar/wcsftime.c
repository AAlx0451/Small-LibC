...
static int iso8601wknum(const struct tm *t) {
    ...
    if (t->tm_mon == 11) {
        ...
        if ((wd == 1 && md >= 29) || (wd == 2 && md >= 30) || (wd == 3 && md == 31)) w = 1;
    }
    return w;
}
    ...
    again:
        switch (*format) {
            ...
            case L'%':
                ...
                *s++ = L'%';
                break;
            case L'a': OUTSTR(nl_langinfo(abday_i[RANGE(0, timeptr->tm_wday, 6)])); break;
            case L'A': OUTSTR(nl_langinfo(day_i[RANGE(0, timeptr->tm_wday, 6)])); break;
            case L'b': case L'h': OUTSTR(nl_langinfo(abmon_i[RANGE(0, timeptr->tm_mon, 11)])); break;
            case L'B': OUTSTR(nl_langinfo(mon_i[RANGE(0, timeptr->tm_mon, 11)])); break;
            case L'c': {
                const char *fmt = nl_langinfo(D_T_FMT);
                if (fmt && *fmt) {
                    wchar_t wfmt[256];
                    if (mbstowcs(wfmt, fmt, 256) == (size_t)-1) return 0;
            ...
            case L'C': if (!wfmt_int((wchar_t**)&s, endp, (1900L + timeptr->tm_year) / 100, 2, L'0')) return 0; break;
            case L'd': OUTNUM2(RANGE(1, timeptr->tm_mday, 31)); break;
            case L'D':
                OUTNUM2(RANGE(0, timeptr->tm_mon, 11) + 1);
                if (s >= endp) return 0; *s++ = L'/';
            ...
            case L'e': {
                ...
                if (v < 10) { 
                    if (s >= endp) return 0; *s++ = L' '; 
                    if (s >= endp) return 0; *s++ = (wchar_t)(L'0' + v); 
                } else OUTNUM2(v);
                break;
            }
            case L'E': case L'O':
                ...
                goto again;
            case L'F':
                if (!wfmt_int((wchar_t**)&s, endp, 1900L + timeptr->tm_year, 4, L'0')) return 0;
                if (s >= endp) return 0; *s++ = L'-';
                OUTNUM2(RANGE(0, timeptr->tm_mon, 11) + 1);
                if (s >= endp) return 0; *s++ = L'-';
                OUTNUM2(RANGE(1, timeptr->tm_mday, 31));
            ...
            case L'g': case L'G': {
                ...
                if (*format == L'G') {
                    if (!wfmt_int((wchar_t**)&s, endp, y, 4, L'0')) return 0;
                } else {
                    OUTNUM2((y % 100 + 100) % 100);
                }
                break;
            }
            case L'H': OUTNUM2(RANGE(0, timeptr->tm_hour, 23)); break;
            case L'I': {
                int i = RANGE(0, timeptr->tm_hour, 23);
                if (i == 0) i = 12; else if (i > 12) i -= 12;
                OUTNUM2(i);
                break;
            }
            case L'j': if (!wfmt_int((wchar_t**)&s, endp, timeptr->tm_yday + 1, 3, L'0')) return 0; break;
            case L'm': OUTNUM2(RANGE(0, timeptr->tm_mon, 11) + 1); break;
            case L'M': OUTNUM2(RANGE(0, timeptr->tm_min, 59)); break;
            case L'n': if (s >= endp) return 0; *s++ = L'\n'; break;
            case L'p': OUTSTR(nl_langinfo(ampm_i[timeptr->tm_hour < 12 ? 0 : 1])); break;
            case L'r': {
            ...
            case L's': {
                ...
                if (!wfmt_int((wchar_t**)&s, endp, (long)mktime(&tmp), 1, L'0')) return 0;
                break;
            }
            case L'S': OUTNUM2(RANGE(0, timeptr->tm_sec, 60)); break;
            case L't': if (s >= endp) return 0; *s++ = L'\t'; break;
            case L'T':
            the_time:
                OUTNUM2(RANGE(0, timeptr->tm_hour, 23));
                if (s >= endp) return 0; *s++ = L':';
            ...
            case L'u': {
                ...
                if (s >= endp) return 0; *s++ = (wchar_t)(L'0' + u);
                break;
            }
            case L'U': OUTNUM2(weeknumber(timeptr, 0)); break;
            case L'V': OUTNUM2(iso8601wknum(timeptr)); break;
            case L'w': if (s >= endp) return 0; *s++ = (wchar_t)(L'0' + RANGE(0, timeptr->tm_wday, 6)); break;
            case L'W': OUTNUM2(weeknumber(timeptr, 1)); break;
            case L'x': {
            ...
            case L'y': OUTNUM2((timeptr->tm_year % 100 + 100) % 100); break;
            case L'Y': if (!wfmt_int((wchar_t**)&s, endp, 1900L + timeptr->tm_year, 4, L'0')) return 0; break;
            case L'z': {
                long off;
                if (timeptr->tm_isdst < 0) break;
                if (!tz_set) { tzset(); tz_set = 1; }
                off = -timezone;
                if (timeptr->tm_isdst > 0) off += 3600;
                off /= 60;
                if (off < 0) { 
                    if (s >= endp) return 0; 
                    *s++ = L'-'; off = -off; 
