    ...
    while (1) {
        if(f->_cnt > 0) { 
            ...
            if(__stdio_fill_impl(f) == 0) {
                if(f->_cnt > 0) { byte_read = *f->_ptr++; f->_cnt--; }
                else { byte_read = EOF; }
            } else { byte_read = EOF; }
        }

        if (byte_read == EOF) {
            if (bytes_consumed == 0) return WEOF;
            f->_flags |= __S_ERR;
            return WEOF;
        }
...
static void _unget_wchar(FILE *f, wint_t wc) {
    ...
    if(f->_base && (f->_ptr - len) >= min_ptr) {
        f->_ptr -= len;
        for(size_t i = 0; i < len; i++) f->_ptr[i] = (unsigned char)buf[i];
        f->_cnt += len;
        f->_flags &= ~__S_EOF;
    }
}
...
int vfwscanf(FILE *restrict stream, const wchar_t *restrict format, va_list arg) {
    ...
    if(!(stream->_flags & __S_STR)) {
        ...
        if(stream->_flags & __S_DIRTY) {
            if(__stdio_flush_impl(stream) == EOF) { stream->_flags |= __S_ERR; _spin_unlock(&stream->_lock); return EOF; }
        }
        if(stream->_flags & __S_WR) { stream->_flags &= ~__S_WR; stream->_cnt = 0; stream->_ptr = stream->_base; }
        if(!(stream->_flags & __S_RD)) { stream->_flags |= __S_RD; if(!(stream->_flags & __S_NBF)) { stream->_cnt = 0; stream->_ptr = stream->_base; } }
    }
    ...
    while(*p) {
        ...
        if(*p == L'*') { flags |= FL_SPLAT; p++; }

        if(iswdigit(*p)) {
            flags |= FL_WIDTH;
            while(iswdigit(*p)) { width = width * 10 + (*p - L'0'); p++; }
            if(width == 0) width = 1;
        } else width = -1;

        if(*p == L'h') { p++; if(*p == L'h') { flags |= FL_SHORTSHORT; p++; } else { flags |= FL_SHORT; } }
        else if(*p == L'l') { p++; if(*p == L'l') { flags |= FL_LONGLONG; p++; } else { flags |= FL_LONG; } }
        else if(*p == L'L') { flags |= FL_DOUBLE; p++; }
        else if(*p == L'z' || *p == L't' || *p == L'j') { flags |= FL_LONG; p++; }

        conversion_char = *p++;

        if(conversion_char != L'c' && conversion_char != L'C' &&
           conversion_char != L's' && conversion_char != L'S' &&
           conversion_char != L'[' && conversion_char != L'n') {
            c = _skip_whitespace_wc(stream, &chars_consumed);
            if(c == WEOF) goto input_failure;
            _unget_wchar(stream, c);
            if(c != WEOF) chars_consumed--;
        ...
        case L'd': case L'i': case L'u': case L'x': case L'X': case L'o': case L'p': {
            int base = 10; int is_neg = 0; int has_digit = 0;
            if(conversion_char == L'i') base = 0;
            else if(conversion_char == L'x' || conversion_char == L'X' || conversion_char == L'p') base = 16;
            else if(conversion_char == L'o') base = 8;

            c = _in_wchar(stream);
            if(c != WEOF) chars_consumed++;
            if(c == WEOF) goto input_failure;

            if(c == L'-') { is_neg = 1; c = _in_wchar(stream); if(c != WEOF) chars_consumed++; if(width > 0) width--; }
            else if(c == L'+') { c = _in_wchar(stream); if(c != WEOF) chars_consumed++; if(width > 0) width--; }
            ...
            while(c != WEOF) {
                if(width == 0) { _unget_wchar(stream, c); if(c != WEOF) chars_consumed--; break; }
                val = -1;
                
                /* Narrow the wchar to ASCII for base parsing */
                if (c < 128) {
                    if(isdigit((char)c)) val = c - L'0';
                    else if(isalpha((char)c)) val = tolower((char)c) - 'a' + 10;
                }
        ...
        case L'f': case L'e': case L'E': case L'g': case L'G': {
            i = 0; c = _in_wchar(stream);
            if(c != WEOF) chars_consumed++;
            if(c == WEOF) goto input_failure;

            if(c == L'-' || c == L'+') { buf[i++] = (char)c; c = _in_wchar(stream); if(c != WEOF) chars_consumed++; if(width > 0) width--; }
            ...
            if(!(flags & FL_SPLAT)) {
                ...
                if(flags & FL_LONG) *va_arg(arg, double *) = val2;
                else if(flags & FL_DOUBLE) *va_arg(arg, double *) = val2;
                else *va_arg(arg, float *) = (float)val2;
                nmatch++;
            }
            break;
        }
	    ...
	    chars_consumed--;
            ...
            while(width != 0) {
                ...
                if(c == WEOF || iswspace(c)) {
                    if(c != WEOF) { _unget_wchar(stream, c); chars_consumed--; }
                    break;
                }
            ...
            if(out_str) {
                if (is_wide) *(wchar_t *)out_str = L'\0';
                else *(char *)out_str = '\0';
            }
        ...
        case L'C': case L'c': {
            ...
            while(width-- > 0) {
                ...
                if(c != WEOF) chars_consumed++;
                if(c == WEOF) {
                    if(read_count == 0 && nmatch == 0) goto input_failure;
                    break;
                }
                if(out_str) {
                    if (is_wide) {
        ...
        case L'[': {
            ...
            if(*p == L'^') { invert = 1; p++; set_start++; }
            if(*p == L']') p++; // Special case: ] as first char
            while(*p && *p != L']') p++;
