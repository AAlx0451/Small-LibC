...
int vfwprintf(FILE *restrict stream, const wchar_t *restrict format, va_list arg) {
    ...
    char temp_buf[512], dummy[4], sgn_char;
    char *str_val;
    wchar_t type;
    uintmax_t uval;
    intmax_t sval;
    ...
    while(*p) {
        ...
        p++;
        flags = 0; width = 0; prec = -1; len_mod = 0;

        while(1) {
            if(*p == L'-') flags |= PAD_RIGHT;
            else if(*p == L'+') flags |= PRINT_SGN;
            else if(*p == L' ') flags |= PRINT_SPC;
            else if(*p == L'#') flags |= PRINT_ALT;
            else if(*p == L'0') flags |= PAD_ZERO;
            else if(*p == L'\'') flags |= PRINT_GROUP;
            else break;
            p++;
        }
        ...
        if(*p == L'h') {
            len_mod = 1; p++;
            if(*p == L'h') { len_mod = 2; p++; }
        } else if(*p == L'l') {
            len_mod = 3; p++;
            if(*p == L'l') { len_mod = 4; p++; }
        } else if(*p == L'L') { len_mod = 5; p++;
        } else if(*p == L'z') { len_mod = 6; p++;
        } else if(*p == L't') { len_mod = 7; p++; }

        type = *p++;
        str_val = NULL; slen = 0;
        uval = 0; sval = 0; fval = 0.0;
        base = 10; is_signed = 0; is_integer = 0; is_float = 0;

        switch(type) {
        case L'd':
        case L'i':
            is_signed = 1; is_integer = 1;
        ...
        case L'u': case L'o': case L'x': case L'X': case L'b':
            is_integer = 1;
            if(type == L'o') { base = 8; flags |= PRINT_OCT; }
            else if(type == L'x') { base = 16; flags |= PRINT_HEX_LO; }
            else if(type == L'X') { base = 16; flags |= PRINT_HEX_UP; }
            else if(type == L'b') { base = 2; }
        ...
        case L'p':
            ...
            base = 16; flags |= (PRINT_HEX_LO | PRINT_ALT);
            is_integer = 1;
            break;

        case L'n': {
            ptr = va_arg(arg, uint32_t *);
            if(ptr) *ptr = (unsigned)total_written;
            continue;
        }
        ...
        case L'c': {
            ...
            if(!(flags & PAD_RIGHT)) while(padding-- > 0) total_written += _out_wchar(stream, L' ');
        ...
        case L's': {
            if(type == L'S' || len_mod == 3) {
                ...
                slen = 0;
                while(wstr[slen]) slen++;
                if(prec >= 0 && slen > prec) slen = prec;

                int padding = width - slen;
                if(!(flags & PAD_RIGHT)) {
                    while(padding-- > 0) total_written += _out_wchar(stream, L' ');
                }
                for(int i = 0; i < slen; i++) total_written += _out_wchar(stream, wstr[i]);
                if(flags & PAD_RIGHT) {
                    while(padding-- > 0) total_written += _out_wchar(stream, L' ');
                }
            } else {
                ...
                while(*s_ptr) {
                    ...
                    if(res == 0) break;
                    if(res == (size_t)-1 || res == (size_t)-2) { s_ptr++; continue; }
                    slen++; s_ptr += res;
                }
                if(prec >= 0 && slen > prec) slen = prec;
        ...
        case L'%':
            temp_buf[0] = '%'; slen = 1; str_val = temp_buf;
            break;

        default:
            temp_buf[0] = (char)type; slen = 1; str_val = temp_buf;
            break;
        }
        ...
        if(is_integer && prec >= 0) {
            ...
            if(prec == 0 && uval == 0 && !(flags & PRINT_ALT && base == 8)) slen = 0;
        }

        pad_len = 0; sgn_char = 0;
        ...
        if((flags & PRINT_ALT) && (uval != 0 || type == L'p')) {
            ...
            else if(base == 16) { prefix = (flags & PRINT_HEX_UP) ? "0X" : "0x"; prefix_len = 2; }
        }
        ...
        if(!(flags & PAD_RIGHT) && !(flags & PAD_ZERO)) {
            while(pad_len-- > 0) total_written += _out_wchar(stream, L' ');
        }

        if(sgn_char) total_written += _out_wchar(stream, (wchar_t)sgn_char);
        for(int k = 0; k < prefix_len; k++) total_written += _out_wchar(stream, (wchar_t)prefix[k]);

        if(!(flags & PAD_RIGHT) && (flags & PAD_ZERO)) {
            while(pad_len-- > 0) total_written += _out_wchar(stream, L'0');
        }

        while(prec_zeros-- > 0) total_written += _out_wchar(stream, L'0');
        
        /* The magical upcast: Ascii numeric chars convert natively to wchar_t */
        for(int i = 0; i < slen; i++) total_written += _out_wchar(stream, (wchar_t)str_val[i]);
